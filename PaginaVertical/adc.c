...
/**
  ...
  */
/*MSG queue for ADC*/
  osMessageQueueId_t mid_Msg_ADC;
  uint32_t enviado_adc;

/* Stack */
const osThreadAttr_t Thread_ADC_attr = {.stack_size = 1024};

/*----------------------------------------------------------------------------
 *      Message Queue creation & usage
 *---------------------------------------------------------------------------*/

static int Init_MsgQueue_ADC (void) {
  mid_Msg_ADC = osMessageQueueNew(5, sizeof(uint32_t), NULL);
  if (mid_Msg_ADC == NULL) {
    return(-1);
  }
...
float ADC_getVoltage(ADC_HandleTypeDef *hadc, uint32_t Channel)
  ...
  */
  ...
  if (HAL_ADC_ConfigChannel(hadc, &sConfig) != HAL_OK)
  {
    ...
    return raw; //Cambiado al valor raw, se hace el cálculo en http_server_cgi
}
...
int init_thADC (void){
  thADC = osThreadNew(getADC, NULL, &Thread_ADC_attr);
  if (thADC == NULL){
    return(-1);
  }
  return(0);
...
void getADC (void *arg) {
  ...
  while (1) {
    ...
    value=ADC_getVoltage(&adchandle , 10); //get values from channel 10->ADC1_IN10
    //value=ADC_ getVoltage(&adchandle , 13 ); //Potenciómetro 2
    
    enviado_adc = value;
    osMessageQueuePut(mid_Msg_ADC, &enviado_adc, 0, 0);
    osDelay(1000);
